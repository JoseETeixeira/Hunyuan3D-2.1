"""Prototype: bake the generated canonical ELEVATIONS directly onto the mesh, one per
cardinal face (front/right/back/left + top from above), skipping MV-Adapter and the gpt
transfer entirely. Renders each face to confirm faces are correct + reference-faithful.

Usage: python webapp/bake_elevations.py <uid>
"""
import os
import sys
...
SZ = 512
BAKE_EXP = 6.0
# side -> (Hunyuan elev, azim). Cardinals at equator; top viewed from ABOVE (negative elev).
...
sheet = Image.new("RGB", (len(CARDINAL) * SZ, 2 * SZ), (12, 12, 12))
d = ImageDraw.Draw(sheet)
for c, (side, el, az) in enumerate(CARDINAL):
    p = f"{out}/{uid}_elev_{side}.png"
    elev = Image.open(p).convert("RGB").resize((SZ, SZ)) if os.path.exists(p) else Image.new("RGB", (SZ, SZ), (40, 40, 40))
    sheet.paste(elev, (c * SZ, 0))
    d.text((c * SZ + 5, 5), f"ELEV {side}", fill=(0, 230, 255))
    sheet.paste(render_color(el, az), (c * SZ, SZ))
    d.text((c * SZ + 5, SZ + 5), f"BAKED {side} cam", fill=(0, 255, 120))
path = f"{out}/{uid}_directelev.png"
sheet.save(path)
print("baked sides:", used)
print("saved", path)
